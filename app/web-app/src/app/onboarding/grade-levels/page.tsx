...
export default function GradeLevelsPage() {
  ...
  const [selected, setSelected] = useState<string[]>(data.gradeLevels)
  ...
  useEffect(() => {
    async function loadData() {
      const [content, instructionData] = await Promise.all([
        getOnboardingContent('grade_levels'),
        getOnboardingInstructions('grade_levels')
      ])
  ...
  const handleNext = () => {
    updateData({ gradeLevels: selected })
  }
